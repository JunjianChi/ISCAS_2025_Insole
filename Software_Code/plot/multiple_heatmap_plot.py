...
# 定义常量
FRAME_START = 300
FRAME_END = 1000  # 包含帧300
FRAME_STEP = 5  # 每隔5帧保存一次
...
def generate_mask_from_image(image_path, target_rows=33, target_cols=15, lower_green=np.array([40, 40, 40]),
...
def plot_pressure_heatmap(data_matrix_0, data_matrix_1, left_pattern, right_pattern, frame_index, vmax=None,
    ...
    ax = sns.heatmap(combined_pressure, cmap=custom_cmap, cbar=False, vmax=vmax, vmin=0,
                     square=True)

    # 自定义色条（如果需要，可以取消注释并调整）
    # cbar = ax.collections[0].colorbar
    # cbar.set_ticks([0, 50, 100, 150, 200])
    # cbar.set_label('Pressure (kPa)', fontsize=12, fontweight='bold')
    # plt.setp(cbar.ax.yaxis.get_ticklabels(), fontsize=10, fontweight='bold')
    ...
    # 提高图像的DPI以提高清晰度
    plt.gcf().set_dpi(300)  # 1000可能导致文件过大，建议使用300或600
...
def main():
    ...
    # 循环处理每个选定的帧
    for frame_number in range(FRAME_START, FRAME_END + 1, FRAME_STEP):
        frame_to_plot = frame_number - FRAME_START  # 相对于切片数据的帧索引

        if frame_to_plot < 0 or frame_to_plot >= len(data_matrix_0):
            print(f"帧索引 {frame_number} 超出范围。可用的帧范围是从 {FRAME_START} 到 {FRAME_END}。")
            continue

        # 定义PNG文件的名称
        save_filename = f'pressure_frame_{frame_number}.png'
