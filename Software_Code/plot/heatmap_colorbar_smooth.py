...
ZOOM_FACTOR_SMOOTH = 1.5  # 平滑和矩阵放大因子
ZOOM_FACTOR_MASK = 1.5    # 模式放大因子，与ZOOM_FACTOR_SMOOTH一致
SIGMA = 1.8
V_MAX = 20  # 根据需要调整为20 kPa
V_MIN = 0    # 设置颜色条的最小值
...
def plot_pressure_heatmap(data_matrix_0, data_matrix_1, left_pattern, right_pattern, frame_index, vmax=V_MAX, vmin=V_MIN,
    ...
    ax = sns.heatmap(combined_pressure, cmap=custom_cmap, cbar=True, vmax=vmax, vmin=vmin,
                     square=True, cbar_kws={'label': 'Pressure (kPa)', 'shrink': 0.5})

    # 自定义色条
    cbar = ax.collections[0].colorbar
    cbar.set_ticks([0, 5, 10, 15, 20])  # 设置刻度为0, 5, 10, 15, 20 kPa
    cbar.set_label('Pressure (kPa)', fontsize=12, fontweight='bold')  # 设置色条标签
    cbar.ax.tick_params(labelsize=10)  # 设置色条刻度标签的字体大小
    plt.setp(cbar.ax.yaxis.get_ticklabels(), fontsize=10, fontweight='bold')  # 加粗刻度标签
...
def main():
    ...
    plot_pressure_heatmap(
        ...
        vmax=V_MAX,  # 设置最大值为20 kPa
        vmin=V_MIN,  # 设置最小值为0 kPa
